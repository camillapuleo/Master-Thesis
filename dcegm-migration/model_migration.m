...
properties (Access=public)
	...
	dus     =	-0.058	; %disutility of staying
	theta		= 1.95 	; %CRRA coefficient (log utility if ==1)
	inc0		= 0.65	; %income equation: constant
	inc1		= 0.04  ; %income equation: age coef
	inc2		= 0.0002;	%income equation: age^2 coef
    minc0		= 0.90	; %income equation: constant
	minc1		= 0.04  ; %income equation: age coef
	minc2		= 0.0002;	%income equation: age^2 coef
    cfloor  =	0.001	; %consumption floor (safety net in retirement)
	lambda  = 0.025 ; %scale of the EV taste shocks 
    Tr = 47 ;    %retirement age
    rr = 0.9;    %replacement rate
end %properties
...
methods (Access=public)
	...
	function w=income(me,it,shock,choice) %income in period it with given normal shock
		%assume it=1 is age=20
		age=it+19;
        %minc0= me.inc0*1.3;
        if it < me.Tr 
            if choice == 1
                ...
                w=exp(me.minc0 + me.minc1*age - me.minc2*age.*age + shock);
            end 
        else
            if choice == 1
                w=(me.inc0 + me.inc1*age - me.inc2*age.*age + exp(0+((me.sigma*me.sigma)/2)))*me.rr;
            else
                w=(me.minc0 + me.minc1*age - me.minc2*age.*age + exp(0+((me.sigma*me.sigma)/2)))*me.rr;
            end 
        end
	end %income
    function w1=budget(me,it,savings,shocks,choice) 
		...
		w1=ones(size(shocks,1),1)*savings(1,:)*(1+me.r)+ ...
			 income(me,it+1,shocks(:,1),choice)*ones(1,size(savings,2));
	end %budget
    ...
    for it=1:me.Tbar
    	if it==1
    		%draw initial wealth form uniform distribution on given interval
                ...
                if it<me.Tr
                    ...
                    me.sims.decision(~choice,it)=1; %others leave
                end
				me.sims.migration_age(me.sims.decision(:,it-1)==1 & me.sims.decision(:,it)==2)=it; %record migration age
                ...
                if it<me.Tr
                    ...
                    me.sims.wealth0(~choice,it)=diag(me.budget(it-1,me.sims.wealth1(~choice,it-1)',me.sims.shock(~choice,it),1));
                end
                %me.sims.wealth0(~choice,it)=diag(me.budget(it-1,me.sims.wealth1(~choice,it-1)',me.sims.shock(~choice,it),2));
                %me.sims.income(:,it)=0;
				me.sims.income(choice,it)=me.income(it,me.sims.shock(choice,it),choice);
                me.sims.income(~choice,it)=me.income(it,me.sims.shock(~choice,it),choice);
        end
        ...
        if it<me.Tr
                ...
                me.sims.prob_stay(:,it)=1;%probability to stay (conditional on being a stayer);
        end
        %simulate migration choice
